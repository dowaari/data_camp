...
```
    ...
    (1) 환율추이를 분석하여 고객에게 환전서비스 제공 
        ...
        - comment: 
            * 연관성 있는 데이터가 딱히 생각나지 않음. domain지식부족
            * 뉴스제목수집 -> 텍스트 -> 단어 -> 환율에 긍정/부정인지 머신러닝
            * 참고: 2014년 빅데이터아카데미 우수사례집, 일일 환율 예측 프로젝트 p22
        
    (2) 텍스트마이닝 및 머신러닝을 활용한 코스피 지수의 예측 (코스피지수예측 알림서비스)
        - 목적: 
        ...
        - 역할분담: 
            * 데이터별 수집 및 전처리, 상관관계 분석하여 유의미한 데이터찾아내기 (5) 8/3 ~ 8
                # 과거지수(1), 경기지표(2), 크롤링(2)
                # 기본 하루단위, 최소 월단위값은 있어야, 텍스트는 일별,사이트별수집하여 관련검색어추출 빈도수 조사 
            * 텍스트 마이닝 -> 감성분석, 논문리뷰 (3) 8/3 ~ 13
            * 머신러닝 예측 모델링 (2) 8/14 ~ 
            * 웹시각화 (1) 8/9 ~ 
            * 백테스팅을 통한 수익률비교 (2) 8/14~       
        - comment: 
